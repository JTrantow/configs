...
#define MAX_RPM	        24000   // cap output RPM
...
typedef struct params {
    ...
    haldata_t *haldata;
    int hal_comp_id;
    int read_initial_done;      // better type choice would be bool
    int old_err_reset; 
    uint16_t old_cmd1_reg;		// copy of last write to FA00 */
    int modbus_ok;              // Counter of successful modbus operations.
...
void  windup(param_pointer p) 
{
    if (p->haldata && *(p->haldata->errorcount)) {
        ...
        fprintf(stderr,"%s: last command register: 0x%04X\n",p->progname, p->failed_reg);
...
int write_data(modbus_t *ctx, haldata_t *haldata, param_pointer p)
{
    ...
    */
    ...
    if (*(haldata->spindle_on) && *(haldata->spindle_fwd) && !*(haldata->spindle_rev)) {
        // spindle forward
        control_reg = CTL_DRIVE_ENABLE | CTL_AUTO_MANUAL | CTL_N_STOP | CTL_RUN_FORWARD;
    } else if (*(haldata->spindle_on) && *(haldata->spindle_rev) && !*(haldata->spindle_fwd)) {
        // spindle reverse
        control_reg = CTL_DRIVE_ENABLE | CTL_AUTO_MANUAL | CTL_N_STOP | CTL_RUN_REVERSE;
    } else {
...
int read_initial(modbus_t *ctx, haldata_t *haldata, param_pointer p)
{
    ...
    uint16_t rated_freq, rated_rpm = 0;
    uint16_t max_freq, min_freq;

    GETPARAM(PR_MAX_SET_SPEED, &max_freq);  // PR_MAX_SET_SPEED	106 or 0x006A.
    GETPARAM(PR_MIN_SET_SPEED, &min_freq);  // PR_MIN_SET_SPEED	107 pr 0x006B.

    GETPARAM(PR_RATED_FREQUENCY, &rated_freq);  // PR_RATED_FREQUENCY 506 or 0x01FA.
    //GETPARAM(PR_RATED_RPM, &rated_rpm);         // PR_RATED_RPM 507 or 0x01FB.  
    ...
    *(haldata->lower_limit_hz) = min_freq/10.0;
    ...
    if (p->report_device) {
        ...
        printf("%s: sw version: %d/0x%04X\n", 
            p->progname, sw_version, sw_version);
        printf("%s: sw sub-version: %d/0x%04X\n", 
            p->progname, sw_subversion, sw_subversion);
        printf("%s: dsp version: %d/0x%%04X\n", 
 ...
 failed:
    ...
    (*haldata->errorcount)++;
    if (p->debug)
        fprintf(stderr, "%s: read_initial: modbus_read_registers(0x%04X): %s\n", 
...
{
    int retval;
    uint16_t curr_param, val;
    static int pollcount = 0;

    if (!p->read_initial_done) {
        // Read the initial parameters once at startup.
        if ((retval = read_initial(ctx, haldata, p))) {
            DBG("read_initial(fail 0x%08X).\n",retval);
            return retval;
        }else{
            DBG("read_initial(success)\n");
            p->read_initial_done = 1;
        }
    }else{
        // Update the polled  parameters. 
        uint16_t status_word, freq;

        GETPARAM(PR_STATUS_WORD, &status_word);  // PR_STATUS_WORD = 1040 or 0x0410.
        *(haldata->status) = status_word;

        GETPARAM(PR_MOTOR_FREQUENCY, &freq);    // PR_MOTOR_FREQUENCY = 501 or 0x01F5
        *(haldata->freq_out) = freq / 10.0;
        DBG("read_data: status=0x%04X freq=%d.\n", status_word, freq);
 ...
 failed:
    ...
    (*haldata->errorcount)++;
    if (p->debug)
	fprintf(stderr, "%s: read_data: modbus_read_registers(0x%04X): %s\n", 
		p->progname, curr_param, modbus_strerror(errno));
    return p->last_errno;
}
...
int set_defaults(param_pointer p)
{
    ...
    *(h->max_speed) = 0;
    ...
    h->looptime = 0.010;
    h->speed_tolerance = 0.05;      // output frequency within 5% of target frequency
...
int main(int argc, char **argv)
{
...
#if (1)
    DBG("Wait a few seconds before starting.\n");
    loop_timespec.tv_sec = 3;
    loop_timespec.tv_nsec = 0;
    ...
    while (connection_state == CONNECTED) {
	    if ((retval = read_data(p->ctx, p->haldata, p))) {
    		p->modbus_ok = 0;
            DBG("connection_state/read_data fail 0x%08X.\n", retval);
	    } else {
		    p->modbus_ok++;
	    }
	    ...
	    if (p->haldata->looptime < 0.001) p->haldata->looptime = 0.010; // Minimum delay. TRY LARGER LOOPTIME???
	    if (p->haldata->looptime > 2.0) p->haldata->looptime = 2.0;     // Maximum delay.
